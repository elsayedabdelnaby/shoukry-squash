...
@section('title', __('dashboard.press'))
        ...
        <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    <!--begin::Page Title-->
                    <h5 class="text-dark font-weight-bold my-1 mr-5">{{ __('dashboard.press') }}
                    </h5>
                    <!--end::Page Title-->
                    <!--begin::Breadcrumb-->
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        <li class="breadcrumb-item text-muted">
                            <a href="{{ route('dashboard.press.index') }}"
                                class="text-muted">{{ __('dashboard.press') }}</a>
                        </li>
                    </ul>
                    <!--end::Breadcrumb-->
                </div>
    ...
    <div class="card card-custom">
        <div class="card-header">
            <div class="card-title">
                ...
                <h3 class="card-label">{{ __('dashboard.press') }}</h3>
            </div>
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="{{ route('dashboard.press.create') }}" class="btn btn-primary font-weight-bolder">
                    <i class="flaticon2-plus-1"></i>
                    {{ __('dashboard.new_press') }}</a>
                <!--end::Button-->
            </div>
        </div>
        <div class="card-body">
                ...
                <tbody>
                    @foreach ($press as $press)
                        <tr>
                            ...
                            <td>
                                {{ $press->show_in_home_page }}
                            </td>
                            <td>
                                <a href="{{ route('dashboard.press.edit', ['press' => $press]) }}"
                                    class="btn btn-sm btn-clean btn-icon" title="{{ __('dashboard.edit') }}">
                                    <i class="la la-edit"></i>
                                </a>
                                <form action="{{ route('dashboard.press.destroy', ['press' => $press]) }}" method="post"
